...
const NotasScreen = ({ navigation }) => {
  ...
  const fetchEstudiantes = async () => {
    try {
      const response = await fetch(`${ip}/EXPO2024/api/services/admin/notas.php?action=readAll`, {
        method: 'POST',
        headers: {
  ...
  const renderObservacion = ({ item }) => (
    <View style={styles.content}>
      <View style={styles.tableContainer}>
        ...
        <View style={styles.tableRow}>
          <Text style={styles.tableLabel}>Nota:</Text>
          <Text style={styles.tableValue}>{item.nota}</Text>
        </View>
        <View style={styles.tableRow}>
          <Text style={styles.tableLabel}>Trimestre:</Text>
          <Text style={styles.tableValue}>{item.trimestre}</Text>
        </View>
        <View style={styles.tableRow}>
          <Text style={styles.tableLabel}>Fecha:</Text>
          <Text style={styles.tableValue}>{item.fecha_calificacion}</Text>
        </View>
      </View>
    </View>
  );
